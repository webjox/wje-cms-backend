...
categoriesRouter.post('/:id/image', 
    security.checkUserScope.bind(this, security.scope.WRITE_PRODUCT_CATEGORIES), 
    koaBody({
        formidable: {
            uploadDir: './upload',
    ...
    async (ctx, next) => {
        const result = await categoriesApi.uploadCategoryImage(ctx, utils.getIdByUrl(ctx.url, 2));
        ctx.body = result;
})

categoriesRouter.put('/:id/image', 
    security.checkUserScope.bind(this, security.scope.WRITE_PRODUCT_CATEGORIES), 
    async (ctx, next) => {
        const result = await categoriesApi.updateCategoryImage(utils.getIdByUrl(ctx.url, 2), ctx.request.body);
        ctx.body = result;
})

categoriesRouter.delete('/:id/image', 
    security.checkUserScope.bind(this, security.scope.WRITE_PRODUCT_CATEGORIES), 
    async ctx => {
