...
settingsRouter.get('/email/templates/:name',
    security.checkUserScope.bind(this, security.scope.READ_SETTINGS),
    async ctx => {
    ctx.body =  await emailTemplatesApi.getEmailTemplate(utils.getIdByUrl(ctx.url, 1));
})

settingsRouter.put('/email/templates/:name',
    security.checkUserScope.bind(this, security.scope.WRITE_SETTINGS),
    async ctx => {
    ctx.body =  await emailTemplatesApi.updateEmailTemplate(utils.getIdByUrl(ctx.url, 1), ctx.request.body);
})

settingsRouter.get('/commerceform',
    security.checkUserScope.bind(this, security.scope.READ_SETTINGS),
    async ctx => {
    ctx.body =  await commerceSettingsApi.retrieveCommerceSettings();
})

settingsRouter.put('/commerceform',
    security.checkUserScope.bind(this, security.scope.WRITE_SETTINGS),
    async ctx => {
    ctx.body =  await commerceSettingsApi.updateCommerceSettings(ctx.request.body);
})

settingsRouter.get('/checkout/fields', 
    security.checkUserScope.bind(this, security.scope.READ_SETTINGS),
    async ctx => {
    ctx.body =  await checkoutFieldsApi.getCheckoutFields();
})

settingsRouter.get('/checkout/fields/:name', 
    security.checkUserScope.bind(this, security.scope.READ_SETTINGS),
    async ctx => {
    ctx.body =  await checkoutFieldsApi.getCheckoutField(utils.getIdByUrl(ctx.url, 1));
})

settingsRouter.put('/checkout/fields/:name',
    security.checkUserScope.bind(this, security.scope.WRITE_SETTINGS),
    async ctx => {
    ctx.body =  await checkoutFieldsApi.updateCheckoutField(utils.getIdByUrl(ctx.url, 1), ctx.request.body);
})

settingsRouter.post('/logo',
    security.checkUserScope.bind(this, security.scope.WRITE_SETTINGS),
    async (ctx, next) => {
    ctx.body =  await settingsApi.uploadLogo(ctx, next);
})

settingsRouter.delete('/logo',
    security.checkUserScope.bind(this, security.scope.WRITE_SETTINGS),
    async (ctx, next) => {
